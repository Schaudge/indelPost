...
def is_covering_target(
    ...
    mut_cigar_list = cigar_ptrn.findall(mut_aln_cigar)

    if len(mut_cigar_list) > 1:
        return False

    repeat_unit = to_minimal_repeat_unit(indel_seq)
    mapped_len = int(mut_cigar_list[0][:-1])
    mut_ref_lt_len = len(mut_ref_lt)
    mut_ref_mid_len = len(mut_ref_mid)
    mut_ref_rt_len = len(mut_ref_rt)
    total_consumption = read_aln_end - read_aln_start + 1
    is_lt_read_consumed = (read_aln_start == 0)
    is_rt_read_consumed = (read_aln_end == read_seq_len - 1)
